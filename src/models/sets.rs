...
impl Model {
    ...
    pub async fn insert(&self, db: &PgPool) -> ModelResult<()> {
        sqlx::query!(
            "INSERT INTO sets (id, creator_id, dj_names, venue, city, event_name, event_date, \
             doors_time, scheduled_start, actual_start, end_time, setlist) VALUES ($1, $2, $3, \
    ...
    pub async fn update(&self, db: &PgPool) -> ModelResult<()> {
        sqlx::query!(
            "UPDATE sets SET dj_names = $1, venue = $2, city = $3, event_name = $4, event_date = \
             $5, doors_time = $6, scheduled_start = $7, actual_start = $8, end_time = $9, setlist \
             = $10 WHERE id = $11",
            &self.dj_names,
            self.venue,
            self.city,
